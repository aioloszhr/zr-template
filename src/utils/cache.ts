...
function setStorage<T = unknown>(
  ...
  if (!key) {
    console.error(`[setStorage]: Failed to set stored data: key ${key} is empty`)
  ...
  try {
    ...
    console.error(`[setStorage]: Failed to set stored data for key '${key}'`, error)
  }
}
...
const removeStorage: RemoveStorageFC = (key, storageType, options) => {
  if (!key) {
    console.error(`[removeStorage]: Failed to remove stored data: key ${key} is empty or undefined`)
