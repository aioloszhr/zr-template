...
export const permissionRouter = (router: Router) => {
  ...
  const isToLogin = (to: RouteLocationNormalized, from: RouteLocationNormalized) =>
    to.path === '/' || from.path === '/login'

  beforeEach((to, from, next) => {
    const token = getStorage<string>(APP_CATCH_KEY.token, 'localStorage')
    const catchRoutePath = getStorage(APP_CATCH_KEY.appMenuKey, 'sessionStorage', {
    ...
    if (token !== null) {
      ...
      if (isToLogin(to, from)) {
        ...
        // 是否为白名单
        if (!isValueType<symbol>(name, 'Symbol') && name && WHITE_ROUTES.includes(name)) {
          next()
        } else {
          // 如果不是白名单，判断是否有权限
